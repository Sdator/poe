...
ParseItemType(ItemDataStats, ItemDataNamePlate, ByRef BaseType, ByRef SubType, ByRef GripType)
{
    ...
    Loop, Parse, ItemDataNamePlate, `n, `r
    {
        ...
        ; all armour types.
        IfInString, A_LoopField, 鎖子甲
        {
            BaseType = Armour
            SubType = BodyArmour
            return
        }
        IfInString, A_LoopField, 披風
        {
            BaseType = Armour
            SubType = BodyArmour
            return
        }
        IfInString, A_LoopField, 之鎧
        {
            BaseType = Armour
            SubType = BodyArmour
            return
        ...
        ; Shields 
        IfInString, A_LoopField, 魔盾
        {
            BaseType = Armour
            SubType = Shield
            return
        }
        IfInString, A_LoopField, 塔盾
        {
            BaseType = Armour
            SubType = Shield
            return
        }        
        IfInString, A_LoopField, 鳶盾
        {
            BaseType = Armour
            SubType = Shield
            return
        }                
        IfInString, A_LoopField, 輕盾
        {
            BaseType = Armour
            SubType = Shield
            return
        }        
        IfInString, A_LoopField, 刺盾
        {
            BaseType = Armour
            SubType = Shield
            return
        }
        IfInString, A_LoopField, 圓盾
        {
            BaseType = Armour
            SubType = Shield
            return
        ...
        IfInString, A_LoopField, 頭籠
        {
            BaseType = Armour
            SubType = Helmet
            return
        }
        IfInString, A_LoopField, 角帽
        {
            BaseType = Armour
            SubType = Helmet
            return
...
GetAffixTypeFromProcessedLine(PartialAffixString)
{
    ...
    Loop, %NumAffixLines%
    {
        ...
        If RegExMatch(AffixLine, PartialAffixString)
        {
...
{    
    ...
    Loop, Parse, ItemDataAffixes, `n, `r
    {
        ...
        IfInString, A_LoopField, 使用驅散身上詛咒
        {
            If (NumSuffixes < 1)
            {
                NumSuffixes += 1
        ...
        IfInString, A_LoopField, 近戰攻擊附加擊退敵人
        {
            If (NumSuffixes < 1)
            {
                NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "當.*補充")
        {
            If (NumPrefixes < 1) 
            {
                NumPrefixes += 1
...
ParseAffixes(ItemDataAffixes, Item)
{
    ...
    {    
        ...
        if RegExMatch(A_LoopField, "增加 .*% 照亮範圍")
        {
            HasIncrLightRadius := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加 .*% 物品數量")
        {
            HasIIQ := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*物理傷害")
        {
            HasIncrPhysDmg := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*命中率.*")
        {
            HasIncrAccuracyRating := A_Index
            Continue
        }
        IfInString, A_LoopField, 命中
        {
            HasToAccuracyRating := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*護甲與閃避.*")
        {
            HasHybridDefences := A_Index
            HasIncrArmourAndEvasion := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*護甲與能量護盾.*")
        {
            HasHybridDefences := A_Index
            HasIncrArmourAndES := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*閃避與能量護盾.*")
        {
            HasHybridDefences := A_Index
            HasIncrEvasionAndES := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*護甲.*")
        {
            HasIncrArmour := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*閃避率.*")
        {
            HasIncrEvasion := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*能量護盾.*")
        {
            HasIncrEnergyShield := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*暈眩回復.*")
        {
            HasStunRecovery := A_Index
            Continue
        }
        if RegExMatch(A_LoopField, "增加.*法術傷害.*")
        {
        ...
        IfInString, A_Loopfield, 可以擁有複數工藝屬性
        {
    ...
    Loop, Parse, ItemDataChunk, `n, `r
    {
        ...
        if RegExMatch(A_LoopField, "增加 .*% 攻擊速度")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 施法速度")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 法術暴擊率")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加.*暴擊率")
        {
        ...
        if RegExMatch(A_LoopField, "增加.*暴擊加成")
        {
            If (ItemSubType == "Quiver" or ItemSubType == "Amulet")
            {
        ...
        if RegExMatch(A_LoopField, "增加 .*% 火焰傷害")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 冰冷傷害")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 閃電傷害")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "減少 .*% 藥劑充能使用")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 藥劑充能獲取")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 藥劑效果持續時間")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 魔力回復速度")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 投射物速度")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "減少 .*% 能力值需求")
        {
            NumSuffixes += 1
        ...
        If RegExMatch(A_LoopField, ".*(冰冷|火焰|閃電)與(冰冷|火焰|閃電)抗性")
        {
        ...
        if RegExMatch(A_LoopField, "增加 .*% 敵人暈眩時間")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "減少 .*% 敵人暈眩門檻")
        {
            NumSuffixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 護甲與閃避")
        {
            ...
            If (HasStunRecovery) 
            {
                ...
                BSRecBracketLevel := 0
                BSRecValue := ExtractValueFromAffixLine(ItemDataChunk, "增加.*暈眩回復")
        ...
        If RegExMatch(A_LoopField, "附加 \d+?\-\d+? 物理傷害")
        {
        ...
        If RegExMatch(A_LoopField, "附加 \d+?\-\d+? 冰冷傷害")
        {
        ...
        If RegExMatch(A_LoopField, "附加 \d+?\-\d+? 火焰傷害")
        {
            If (ItemSubType == "Amulet" or ItemSubType == "Ring")
            {
        ...
        If RegExMatch(A_LoopField, "附加 \d+?\-\d+? 閃電傷害")
        {
            If (ItemSubType == "Amulet" or ItemSubType == "Ring")
            {
        ...
        If RegExMatch(A_LoopField, "反射 \d+? 近戰物理傷害")
        {
            NumPrefixes += 1
        ...
        if RegExMatch(A_LoopField, "裝備上.*寶石")
        ;IfInString, A_LoopField, to Level of Socketed
        {
            ...
            Else
            {
                If (InStr(A_LoopField, "召喚物"))
                {
                    ValueRange := LookupAffixData("data\GemLevel_Minion.txt", ItemLevel, CurrValue, "", CurrTier)
                }
                Else If (InStr(A_LoopField, "火焰") or InStr(A_LoopField, "冰冷") or InStr(A_LoopField, "閃電"))
                {
                    ValueRange := LookupAffixData("data\GemLevel_Elemental.txt", ItemLevel, CurrValue, "", CurrTier)
                }
                Else If (InStr(A_LoopField, "近戰"))
                {
                    ValueRange := LookupAffixData("data\GemLevel_Melee.txt", ItemLevel, CurrValue, "", CurrTier)
                }
            }
        ...
        IfInString, A_LoopField, 物理攻擊傷害偷取
        {
            NumPrefixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 武器元素傷害")
        {
            NumPrefixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 藥劑魔力回復速度")
        {
            NumPrefixes += 1
        ...
        if RegExMatch(A_LoopField, "增加 .*% 藥劑生命回復速度")
        {
            NumPrefixes += 1
    ...
    Loop, Parse, ItemDataChunk, `n, `r
    {
        ...
        if RegExMatch(A_LoopField, "增加.*物理傷害")
        {
            ...
            If (HasToAccuracyRating)
            {
                ...
                {
                    LRValue := ExtractValueFromAffixLine(ItemDataChunk, "增加.*照亮範圍")
       ...
       SimpleIPDPrefix:
        ...
        if RegExMatch(A_LoopField, "增加.*暈眩回復")
        {
            ...
            If (HasHybridDefences)
            {
                ...
                If (Not IsValidBracket(BSRecAffixBracket))
                {
                    ...
                    {
                        PartialAffixString := "增加 .*% 護甲與閃避"
                    }
                    If (HasIncrEvasionAndES) 
                    {
                        PartialAffixString := "增加 .*% 閃避與能量護盾"
                    }
                    If (HasIncrArmourAndES)
                    {
                        PartialAffixString := "增加 .*% 護甲與能量護盾"
                    }
        ...
        IfInString, A_LoopField, 點命中
        {
            ...
            ValueRangeAR := "0-0"
            AffixType := ""
            IPDAffixType := GetAffixTypeFromProcessedLine("增加.*物理傷害")
            If (HasIncrLightRadius and Not HasIncrAccuracyRating) 
            {
                ; "of Shining" and "of Light"
                LightRadiusValue := ExtractValueFromAffixLine(ItemDataChunk, "增加 .*% 照亮範圍")
...
ExtractValueFromAffixLine(ItemDataChunk, PartialAffixString)
{
    Loop, Parse, ItemDataChunk, `n, `r
    {
        ...
        if RegExMatch(A_LoopField, PartialAffixString)
        {
            return CurrValue
        }
    }
...
PostProcessData(ParsedData)
{
    ...
    If (Opts.CompactAffixTypes > 0)
    {
        ...
        Loop, %ParsedDataChunks0%
        {
            ...
            CurrChunk := RegExReplace(CurrChunk, "Prefix", "前")                        
						CurrChunk := RegExReplace(CurrChunk, "Item Level", "物品等級")
            CurrChunk := RegExReplace(CurrChunk, "Base Level", "需求等級")
            CurrChunk := RegExReplace(CurrChunk, "Max Sockets", "最大連線")
            CurrChunk := RegExReplace(CurrChunk, "Phys DPS", "物理傷害")
            CurrChunk := RegExReplace(CurrChunk, "Elem DPS", "元素傷害")
            CurrChunk := RegExReplace(CurrChunk, "Total DPS", "總體傷害")
            CurrChunk := RegExReplace(CurrChunk, "Q20 DPS", "Q20 傷害")
            CurrChunk := RegExReplace(CurrChunk, "Affixes", "詞綴")
...
{
    If (InStr(ItemName, "碎片"))
    {
        IsShard := True
        ItemName := "Orb of " . SubStr(ItemName, 1, -StrLen("碎片"))
    }
    If (InStr(ItemName, "卷軸"))
    {
        IsFragment := True
        ItemName := "知識卷軸"
    }
    StackSize := SubStr(ItemStats, StrLen("堆疊數量:  "))
...
{    
    ...
    {    
        RarityLevel := 0
        Item.Level := ParseItemLevel(ItemDataText, "等級:")        
    ...
    Item.IsUnsetRing := (Item.IsRing and InStr(ItemData.NamePlate, "潛能之戒"))    
    ...
    Item.HasEffect := (InStr(ItemData.PartsLast, "使用"))
    ...
    If (Item.IsUnidentified and (Item.Name != "知識卷軸") and Not Item.IsMap)
    {
